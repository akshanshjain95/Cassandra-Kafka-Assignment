...
import edu.knoldus.CassandraCrud.cassandraConn

import scala.collection.JavaConverters._
import org.apache.kafka.clients.consumer.{ConsumerRecords, KafkaConsumer}

object ConsumerExample extends App with CassandraProvider {
  ...
  while (true) {
    ...
    cassandraConn.execute(s"CREATE TABLE IF NOT EXISTS hashtag (id timestamp PRIMARY KEY, hashtag text) ")
    for (record <- records.asScala) {
      cassandraConn.execute(
        s"INSERT INTO hashtag (id, hashtag) VALUES (dateOf(now()), '${record.value()}')")
    }
